...
async function rollbackCollection(db, collectionName) {
  ...
  for (let i = 0; i < documents.length; i += batchSize) {
    ...
    for (const doc of batch) {
      ...
      // Restore function_ids if it exists in connected_tools
      if (connected_tools.function_ids !== undefined) {
        setFields.function_ids = connected_tools.function_ids;
      }

      // Restore connected_agents if it exists in connected_tools
      if (connected_tools.connected_agents !== undefined) {
        setFields.connected_agents = connected_tools.connected_agents;
      }

      // Restore built_in_tools if it exists in connected_tools
      if (connected_tools.built_in_tools !== undefined) {
        setFields.built_in_tools = connected_tools.built_in_tools;
      }

      // Restore variables_path if it exists in connected_tools
      if (connected_tools.variables_path !== undefined) {
        setFields.variables_path = connected_tools.variables_path;
      }
      ...
      // Restore doc_ids if it exists in connected_tools
      if (connected_tools.doc_ids !== undefined) {
        setFields.doc_ids = connected_tools.doc_ids;
      }
