 ...
 * Moves prompt_enhancer_percentage and criteria_check into ai_updates object
 */
...
async function migrateAiUpdatesField() {
  ...
  try {
    ...
    const bridgeVersionsCollection = db.collection("bridgeversions");
    ...
    // Migration for bridge versions
    console.log("\nStarting migration for bridge versions...");
    const versionCursor = bridgeVersionsCollection.find({});
    const versionBatch = [];
    let versionProcessed = 0;

    for await (const doc of versionCursor) {
      versionProcessed++;

      // Skip if ai_updates already exists
      if (doc.ai_updates) {
        console.log(`Skipping bridge version ${doc._id} - ai_updates already exists`);
        continue;
      }
      ...
      // Process batch every 100 documents
      if (versionBatch.length >= 100) {
        await bridgeVersionsCollection.bulkWrite(versionBatch);
        console.log(`Processed batch of ${versionBatch.length} bridge versions (total: ${versionProcessed})`);
        versionBatch.length = 0; // Clear batch
      }
    }

    // Process remaining bridge versions
    if (versionBatch.length > 0) {
      await bridgeVersionsCollection.bulkWrite(versionBatch);
      console.log(`Processed final batch of ${versionBatch.length} bridge versions (total: ${versionProcessed})`);
    }

    console.log(`Bridge version migration completed. Processed ${versionProcessed} documents.`);
    ...
    const versionCount = await bridgeVersionsCollection.countDocuments({ ai_updates: { $exists: true } });

    console.log("\n=== Migration Summary ===");
    console.log(`Configurations with ai_updates: ${configCount}`);
    console.log(`Bridge versions with ai_updates: ${versionCount}`);
    ...
    const oldVersionCount = await bridgeVersionsCollection.countDocuments({
      $or: [{ prompt_enhancer_percentage: { $exists: true } }, { criteria_check: { $exists: true } }]
    });

    console.log(`Configurations with old fields remaining: ${oldConfigCount}`);
    console.log(`Bridge versions with old fields remaining: ${oldVersionCount}`);

    if (oldConfigCount === 0 && oldVersionCount === 0) {
