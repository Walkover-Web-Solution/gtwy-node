...
function buildSchemaFromTemplateFormat(templateFormat, typeOverrides = {}, variables = {}) {
    ...
    rootKeys.forEach((key) => {
        ...
        const getFieldSchema = (f, context) => {
            if (actionDataFields.has(f) || f === "actionData") return getActionDataSchema(f);
            if (onClickTypeFieldNames.has(f)) return { type: "string", enum: ON_CLICK_ACTION_TYPES };
        ...
        if (info.type === 'array') {
            ...
            } else if (onClickTypeFieldNames.has(key)) {
                properties[key] = { type: "string", enum: ON_CLICK_ACTION_TYPES };
            } else {
