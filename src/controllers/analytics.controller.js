...
const getEmbedAnalytics = async (req, res, next) => {
  try {
    ...
    const isValidEmbedFolder = folder && String(folder.org_id) === String(org_id);
    const isEmbedFolderType = folder?.type === "embed";
    if (!isValidEmbedFolder || !isEmbedFolderType) {
      res.locals = {
        success: false,
        message: !isValidEmbedFolder ? "Embed folder not found" : "Folder is not an embed integration"
      };
      req.statusCode = !isValidEmbedFolder ? 404 : 400;
      return next();
    }

    const window = analyticsService.computeWindow({ range, start_date, end_date, interval });
    const agents = await configurationService.getEmbedFolderAgents(org_id, folder_id, filterUserId);
    const userMapPromise = embedAnalyticsService.loadUserMapForAgents(org_id, agents);
